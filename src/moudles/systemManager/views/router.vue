<template>
  ...
  <imp-panel> 
    <h3 class="box-title" slot="header" style="width: 100%;">
      <el-button type="primary" suffix-icon="el-icon-plus" @click="newAdd">新增</el-button>
      <el-button type="danger" suffix-icon="el-icon-delete" @click="batchDelete">删除</el-button>
      <el-button type="success" suffix-icon="el-icon-delete" @click="createAnsyRouter">生成动态路由</el-button>
      <el-button type="success" suffix-icon="el-icon-delete" @click="npmRunBuild" :loading="buildLoading">页面打包</el-button>
    </h3>
    <el-row slot="body" :gutter="18" style="margin-bottom: 20px;">
          ...
          <div class="text item">
            <el-form :model="form" ref="form">
              <el-form-item label="父级" :label-width="formLabelWidth">
                ...
                <el-select-tree v-model="form.parent_id" :treeData="router.routerList" :propNames="defaultProps" clearable
                                placeholder="请选择父级">
                </el-select-tree>
              </el-form-item>
              ...
              <el-form-item label="" :label-width="formLabelWidth">
                ...
                <el-button type="danger" @click="deleteSelected" suffix-icon="el-icon-delete" v-show="form.id && form.id!=null">删除
                </el-button>
              </el-form-item>
            </el-form>
          </div>
            ...
            <div class="select-tree icons-container">
              <el-scrollbar
                ...
                <div class="icons-wrapper">
                  <div v-for="item of iconsMap" :key="item" @click="handleClipboard(item)">
  ...
  export default {
    ...
    data(){
      return {
        ...
        maxId: 7000000,
        menuTree: [
          {"id":33,"parent_id":null,"sort":0,"name":"仪表盘","path":"/index","icon":"fa fa-dashboard","children":[],"hidden":"1"},
          
          {"id":102,"parent_id":null,"sort":3,"name":"测试2","path":"/test/2","icon":"fa fa-download","children":[{"id":103,"parent_id":102,"sort":0,"name":"测试2-1","path":"/test/2/1","icon":"fa fa-image","children":[],"hidden":"1"}],"hidden":"1"},
          ],
        form: {
          id: null,
          name: '',
    ...
    methods: {
      ...
      batchDelete(){
        ...
        this.$confirm('确定删除?', '提示', {
          ...
          let params = {resData:checkKeys.join(',')},self = this
          this.$store.dispatch('DELETE_ROUTER',params)
          .then(res=>{
            this.$message('操作成功');
          }).catch(error=>{
            this.$message('操作失败');
      ...
      onSubmit(){
        let params = this.form,self = this
        // 判断是 修改还是添加
        ...
        if(this.form.id>0){
          this.$store.dispatch('updateRouter',{...params,resource_id:params.id})
          .then(res=>{
              // 再次刷新数据
              self.load();
          });
        }else{
          this.$store.dispatch('addRouter',params)
          .then(res=>{
              // 再次刷新数据
              self.load();
          });
...
.icons-container { margin: 10px 20px 0; overflow: hidden; .icons-wrapper { margin: 0 auto; } .icon-item { margin: 20px; height:
110px; text-align: center; width: 110px; float: left; font-size: 30px; color: #24292e; cursor: pointer; } span { display:
block; font-size: 24px; margin-top: 10px; } }
